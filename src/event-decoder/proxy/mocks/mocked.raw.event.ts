import { RawEvent } from '../../raw.event';

export const rawAddLiquidityProxyEvent = new RawEvent({
    address: 'erd1qqqqqqqqqqqqqpgqrc4pg2xarca9z34njcxeur622qmfjp8w2jps89fxnl',
    identifier: 'addLiquidityProxy',
    topics: [
        'YWRkX2xpcXVpZGl0eV9wcm94eQ==',
        'V0VHTEQtYmQ0ZDc5',
        'TEtNRVgtYWFiOTEw',
        'EsYhAqZsYG90Ayj/HRxxwLx6xXPP0x15v0iGiPoFqb8=',
        'AAAAAAAAAAAFAOvTBMLzSms/alfBM6t7jG+B3EAVVIM=',
        'AoM=',
    ],
    data: '',
});
export const rawRemoveLiquidityProxyEvent = new RawEvent({
    address: 'erd1qqqqqqqqqqqqqpgqrc4pg2xarca9z34njcxeur622qmfjp8w2jps89fxnl',
    identifier: 'removeLiquidityProxy',
    topics: [
        'cmVtb3ZlX2xpcXVpZGl0eV9wcm94eQ==',
        'V0VHTEQtYmQ0ZDc5',
        'TUVYLTQ1NWM1Nw==',
        '61kllOfng7p9cL0lnjcuSRt6KNib+USuMhO3H6+beB4=',
        'AAAAAAAAAAAFAOvTBMLzSms/alfBM6t7jG+B3EAVVIM=',
        'AoM=',
    ],
    data: '',
});

export const rawEnterFarmProxyEvent = new RawEvent({
    address: 'erd1qqqqqqqqqqqqqpgqrc4pg2xarca9z34njcxeur622qmfjp8w2jps89fxnl',
    identifier: 'enterFarmProxy',
    topics: [
        'ZW50ZXJfZmFybV9wcm94eQ==',
        'TEtMUC0wM2EyZmE=',
        '0zi4BGc4NAjFzsJQ517E32CkFYJkY70XJUiDIlm7wsw=',
        'AAAAAAAAAAAFACdciLSjr9Xhcz7JYC82SfBL2UYCVIM=',
        'AoM=',
    ],
    data: '',
});
export const rawExitFarmProxyEvent = new RawEvent({
    address: 'erd1qqqqqqqqqqqqqpgqrc4pg2xarca9z34njcxeur622qmfjp8w2jps89fxnl',
    identifier: 'exitFarmProxy',
    topics: [
        'ZXhpdF9mYXJtX3Byb3h5',
        'TEtMUC0wM2EyZmE=',
        'AHMJaooY6pAuq+nHGuY/Z02bFr1WMtm9KdF5J6u8ziY=',
        'AAAAAAAAAAAFACdciLSjr9Xhcz7JYC82SfBL2UYCVIM=',
        'AoM=',
    ],
    data: '',
});
export const rawClaimRewardsProxyEvent = new RawEvent({
    address: 'erd1qqqqqqqqqqqqqpgqrc4pg2xarca9z34njcxeur622qmfjp8w2jps89fxnl',
    identifier: 'claimRewardsProxy',
    topics: [
        'Y2xhaW1fcmV3YXJkc19mYXJtX3Byb3h5',
        'TEtGQVJNLTlkMWVhOA==',
        'vgNUNDQZkcAKj4vji3XJsJN5ZxZCTkhcKdLihkK6oBA=',
        'AAAAAAAAAAAFACdciLSjr9Xhcz7JYC82SfBL2UYCVIM=',
        'AoM=',
    ],
    data: '',
});
